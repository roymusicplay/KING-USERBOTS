...
async def translate(_, message):
    ...
    if message.reply_to_message and (
        ...
        try:
            ...
            await message.edit_text(f"Error: `{str(err)}`", parse_mode="Markdown")
            return
    else:
        ...
        try:
            ...
            await message.edit_text("Error: `{}`".format(str(err)), parse_mode="Markdown"
            )
            return

    await message.edit_text(f"**Translated:**\n```{tekstr.text}```\n\n**Detected Language:** `{(await trl.detect(text))}`",
        parse_mode="Markdown",
    )
